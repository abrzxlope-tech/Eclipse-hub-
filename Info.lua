-- ====================================================================================================
-- ECLIPSE HUB: MASTER LOADER (CON VISOR 3D, NOMBRE Y OPCIONES INTEGRADAS)
-- ====================================================================================================
...
TitleLabel.Text = 'ECLIPSE HUB <font color="#a250ff">[PRO]</font> | User: <font color="#50ffd2">' .. LocalPlayer.Name .. '</font>'
...
-- 5. Menú Lateral (Sidebar) y Visor 3D del Avatar
...
-- Contenedor del Visor 3D (ViewportFrame)
...
-- Lógica para renderizar tu personaje en 3D dentro del menú lateral
pcall(function()
...
-- Función auxiliar para cargar módulos externos desde GitHub de forma dinámica
...
-- 6. Función para los botones del menú lateral
...
-- 7. Conexión de Botones a los Módulos de GitHub
CreateSidebarButton("⚡ Auto Steal", 145, function()
    local autoStealMod = loadModule("https://raw.githubusercontent.com/abrzlxope-tech/eclipse-hub/main/autosteal.lua")
    if autoStealMod and autoStealMod.Init then
        autoStealMod.Init(TabsContainer)
    else
        -- Panel de respaldo por si el módulo externo aún no conecta
        for _, child in ipairs(TabsContainer:GetChildren()) do child:Destroy() end
        local fallbackTitle = Instance.new("TextLabel")
        fallbackTitle.Size = UDim2.new(1, -20, 0, 30)
        fallbackTitle.Position = UDim2.new(0, 10, 0, 10)
        fallbackTitle.BackgroundTransparency = 1
        fallbackTitle.Text = "AUTO STEAL MODULE (LOCAL FALLBACK)"
        fallbackTitle.TextColor3 = Color3.fromRGB(220, 130, 255)
        fallbackTitle.TextSize = 14
        fallbackTitle.Font = Enum.Font.GothamBold
        fallbackTitle.TextXAlignment = Enum.TextXAlignment.Left
        fallbackTitle.Parent = TabsContainer
    en
end)
